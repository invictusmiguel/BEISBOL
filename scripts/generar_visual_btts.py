# sampro_v6/beisbol/scripts/generar_visual_btts.py
...
img_dir = os.path.join(base_dir, 'visuales', 'imagenes', 'btts')
os.makedirs(img_dir, exist_ok=True)

file_in = os.path.join(pred_dir, f"predicciones_btts_clasificadas_{today}.json")

if not os.path.exists(file_in):
    print("❌ No se encontraron picks clasificados de BTTS.")
    exit()
...
for pick in picks:
    ...
    img = Image.new("RGB", (600, 260), color=(20, 20, 20))
    ...
    nombre = f"{equipos.replace(' ', '_')}_BTTS.png"
    ruta = os.path.join(img_dir, nombre)
    img.save(ruta)
    print(f"✅ Imagen guardada: {ruta}")

print("🎯 Visuales del mercado BTTS generadas.")
